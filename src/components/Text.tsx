...
const StyledText = styled.span<TextProps>`
  ...
  @media (max-width: ${({ theme }) => theme.breakpoints.desktop}) {
    font-size: ${({ theme, fontSize }) => fontSize ? theme.fontSizes.mobile[fontSize] : '1.125rem'};
  }
`;

const Text: React.FC<TextProps> = ({ children, as = 'span', color = 'white', fontWeight = 'regular', fontSize = 'regular', $textAlign }) => (
	<StyledText as={as} color={color} fontWeight={fontWeight} fontSize={fontSize} $textAlign={$textAlign}>
		{children}
	</StyledText>
);
